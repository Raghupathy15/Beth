...
class Tickets(models.Model):
	...
	service_category = fields.Selection([('repair','Repair'),('warranty','Warranty'),('maintanance','Maintanance')],
	...
	assign_id = fields.Many2one('res.users',string='Assign to',required=True)
...
class ServiceProducts(models.Model):
	...
	picking_id = fields.Many2one('stock.picking',string='Delivery Ref No')
	warranty_months = fields.Char(string="Warranty Months",readonly=True)
	warranty_from = fields.Date(string="Warranty From",readonly=True)
	warranty_to = fields.Date(string="Warranty To",readonly=True)
	...
	def _check_product_uom_qty(self):
		for product in self:
			...
			pick = self.env['stock.picking'].search([('name','=',product.picking_id.name)])
			if pick:
				for picking in pick.move_ids_without_package:
					if product.product_uom_qty > picking.quantity_done:
						raise UserError('Product Qty should be equal or less than delived qty, For details refer %s.'% product.picking_id.name)
	...
	def onchange_product_id(self):
		for rec in self:
			if rec.product_id:
				...
				pick = self.env['stock.picking'].search([('partner_id','=',rec.header_id.customer_id.id)],order='id desc',limit=1)
				if pick:
					for picking in pick.move_ids_without_package:
						if picking.product_id == rec.product_id:
							rec.picking_id = pick
							rec.warranty_months = picking.warranty_months
							rec.warranty_from = picking.warranty_from
							rec.warranty_to = picking.warranty_to

	@api.onchange('picking_id')
	def onchange_picking_id(self):
		for rec in self:
			if rec.picking_id:
				pick = self.env['stock.picking'].search([('name','=',rec.picking_id.name)])
				if pick:
					if pick.partner_id.id == rec.header_id.customer_id.id:
						for picking in pick.move_ids_without_package:
							if picking.product_id == rec.product_id:
								rec.warranty_months = picking.warranty_months
								rec.warranty_from = picking.warranty_from
						...
						raise UserError('The "Delivery Ref No" is not matching for this customer !!!')
...
class TicketActivity(models.Model):
	...
	activity_id = fields.Many2one('mail.activity.type',string='Activity',required=True)
	date = fields.Date('Date',default=fields.Date.context_today,readonly=True)
	header_id = fields.Many2one('tickets')